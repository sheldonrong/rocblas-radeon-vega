...
ROCM_BRANCH="${ROCM_BRANCH:-release/rocm-rel-7.2}"
REPO_URL="${REPO_URL:-https://github.com/ROCm/rocm-libraries.git}"
SRC_ROOT="${SRC_ROOT:-$HOME/src/rocm-libraries-7.2-gfx900-gfx906}"
...
apt update -y
apt install -y cmake libmsgpack-dev
...
if [[ ! -d rocm-libraries/.git ]]; then
  git clone --branch "$ROCM_BRANCH" --depth 1 "$REPO_URL" rocm-libraries
else
  git -C rocm-libraries fetch --depth 1 origin "$ROCM_BRANCH"
  git -C rocm-libraries checkout "$ROCM_BRANCH"
  git -C rocm-libraries pull --ff-only || true
fi

PROJECT_DIR="$SRC_ROOT/rocm-libraries/projects/rocblas"
[[ -d "$PROJECT_DIR" ]] || die "projects/rocblas not found"
